...
<body class="bg-gradient-to-t from-[#fbc2eb] to-[#a6c1ee] min-h-screen">
    ...
    <div class="flex justify-center items-center mb-10">
        <div class="w-5/6 p-6 shadow-lg bg-white rounded-md mt-10">
            <h1 class="text-center font-bold text-5xl mt-2 mb-10">List Daftar Karyawan</h1>
            <div class="overflow-x-auto">
                <div class="items-left">
                    <form method="GET" action="{{ url('/admin/daftarkaryawan') }}" class="flex items-center mb-5">
                        <div class="flex gap-2">
                            <input type="text" name="search" placeholder="Nama / NIP / Divisi..."
                    ...
                    <button class="bg-[#a6c1ee] text-white px-5 py-2 rounded-md hover:bg-[#87acec] mb-5"><a href="/admin/tambahkaryawan">Tambah Karyawan</a></button>
                </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="px-4 py-2 border">
                                <img src="{{ asset('storage/' . $karyawan->foto) }}" alt="foto" class="h-40 w-30 object-cover rounded">
                            </td>
                            ...
                            <td class="px-4 py-2 border">{{ $karyawan->alamat }}</td>
                            <td class="px-4 py-2 border">{{ $karyawan->no_telp }}</td>
                            <td class="px-4 py-2 border">{{ $karyawan->email }}</td>
                            <td class="px-4 py-2 border">
                                <div class="flex gap-2">
                                    <a href="/admin/editkaryawan/{{ $karyawan->id }}" class="px-3 py-1 bg-blue-400 text-white rounded hover:bg-blue-500">
                                        Edit
                                    </a>
                                    <form action="/admin/hapuskaryawan/{{ $karyawan->id }}" method="post">
                                        @method('delete')
                                        @csrf
