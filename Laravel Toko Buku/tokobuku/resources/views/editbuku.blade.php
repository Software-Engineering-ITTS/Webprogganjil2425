...
<body class="bg-gradient-to-t from-[#fbc2eb] to-[#a6c1ee] h-screen">
    ...
    <div class="flex justify-center items-center">
        <div class="w-2/3 p-6 shadow-lg bg-white rounded-md mt-10">
            <h1 class="text-center font-bold text-5xl mt-2 mb-5">Form Edit Buku</h1>
            <form action="/admin/editbuku/{{ $buku->id }}" method="post" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="mb-3 mt-3">
                    <label for="judul">Judul</label>
                    <input type="text" name="judul" id="judul" class="border rounded-md w-full text-base px-2 py-1" placeholder="Masukkan judul buku..." value="{{ $buku->judul }}" require>
                </div>
                <div class="mb-5">
                    <label for="penulis">Penulis</label>
                    <input type="text" name="penulis" id="penulis" class="border rounded-md w-full text-base px-2 py-1" placeholder="Masukkan penulis buku..." value="{{ $buku->penulis }}" require>
                </div>
                <div class="mb-5">
                    <label for="harga">Harga</label>
                    <input type="number" name="harga" id="harga" class="border rounded-md w-full text-base px-2 py-1" placeholder="Masukkan harga buku..." value="{{ $buku->harga }}" require>
                </div>
                <div class="mb-5">
                    <label for="stok">Stok</label>
                    <input type="number" name="stok" id="stok" class="border rounded-md w-full text-base px-2 py-1" placeholder="Masukkan stok buku..." value="{{ $buku->stok }}" require>
                </div>
                <div class="mb-5">
                    <label for="foto">Foto</label>
                    <input type="file" name="foto" id="foto" class="border rounded-md w-full text-base px-2 py-1" require>
                    <img src="{{ asset('storage/' . $buku->foto) }}" alt="Foto Buku" width="150" class="mt-3">
                </div>
