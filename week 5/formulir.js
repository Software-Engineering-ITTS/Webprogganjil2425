document.addEventListener("DOMContentLoaded", function() {
    ...
    provinsiDropDown.addEventListener('change',function(){
        kabupatenDropDown.innerHTML = `<option value="2">Kabupaten/kota</option>`
         kecamatanDropDown.innerHTML =  `<option value="3">Kecamatan</option>`
         kelurahanDropDown.innerHTML =`<option value="4">Kelurahan</option>`
...
function getKelurahan(){
    const kelurahanDropDown = document.getElementById("kelurahan")
    fetch(`https://www.emsifa.com/api-wilayah-indonesia/api/villages/3402110.json`)
    .then(response=>response.json())
    .then(response=>{
        let kelurahan = '';
