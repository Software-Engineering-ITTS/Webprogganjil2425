...
function App() {
  ...
  const fetchUser = async () => {
    try {
      const response = await fetch(
        "http://localhost/phpproject/backend/lihatdataa.php"
  ...
  const deleteData = async (userid) => {
    try {
      await fetch("http://localhost/phpproject/backend/hapusdataa.php", {
        method: "POST",
  ...
  const onsubmit = async (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      const url = isEdit
        ? "http://localhost/phpproject/backend/editdataa.php"
        : "http://localhost/phpproject/backend/simpandataa.php";
