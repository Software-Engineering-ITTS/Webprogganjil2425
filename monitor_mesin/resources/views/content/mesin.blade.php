    ...
    <div class="row">
        <div class="col-md-12 mb-10">
            <div class="relative flex items-center">
                <h1 class="font-medium text-lg text-gray-900 dark:text-white mb-4">Form Mesin</h1>
            ...
            @if (Auth()->user()->role == 'admin')
                <form class="w-full md:max-w-5xl" id="mesinForm" action="/mesin" method="POST"
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:gap-6">
                        ...
                        <div class="mb-5 md:mb-0 w-full md:w-1/2">
                            ...
                            <input type="text" id="nama" name="nama"
                                ...
                                placeholder="Enter Name Mesin" required>
                        </div>
                        ...
                        <div class="mb-5 md:mb-0 w-full md:w-1/2">
                            ...
                            <select
                                class="category_id bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                name="category_id" id="category_id" aria-label="Default select example" required>
                                <option value="" @readonly(true)>Enter Select Category</option>
                        ...
                        <div class="mb-5 md:mb-0 w-full md:w-1/2">
                            <label for="status"
                                class="inline-flex cursor-pointer mt-7 text-sm font-medium text-gray-900 dark:text-white">
                                <input id="status" name="status" type="checkbox" value="Mesin Off" class="sr-only peer"
                        ...
                        <td>
                            <div class="flex" style="align-items:center">
                                <a href="javascript:void(0)" class="edit-btn" data-id="{{ $item->id }}"
                                    data-nama="{{ $item->nama }}" data-category-id="{{ $item->category_id }}"
                                        ...
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                            ...
                                            <div class="p-4 md:p-5 text-center">
                                                ...
                                                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are
                                                    you
                                                    sure you want to delete the mesin by name <span
                                                        class="font-medium">{{ $item->nama }}</span>?</h3>
                                                <form method="POST" action="{{ route('mesin.destroy', $item->id) }}"
                                                    class="inline-block">
                                                    @csrf
                                                    @method('DELETE')
...
@section('js')
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            editButtons.forEach(button => {
                button.addEventListener('click', (event) => {
                    ...
                    // Pilih kategori yang sesuai berdasarkan category_id
                    const categoryDropdown = form.querySelector('#category_id');
                    if (categoryDropdown && categoryId) {
                        categoryDropdown.value = categoryId; // Pilih kategori yang sesuai
                    }

                    // Atur status checkbox berdasarkan data-status
                    const statusCheckbox = form.querySelector('#status');
                    if (statusCheckbox) {
                        // Cek apakah status 'mesin on' atau 'mesin off'
                        if (status === 'Mesin On') {
                            statusCheckbox.checked = true; // Centang checkbox jika status mesin on
                            statusCheckbox.value = 'Mesin On';
                        } else {
                            statusCheckbox.checked =
                                false; // Jangan centang checkbox jika status mesin off
                            statusCheckbox.value = 'Mesin Off';
                        }
                    }
                });
            });
    ...
    <script>
        function updateStatus() {
            ...
            statusCheckbox.value = statusCheckbox.checked ? 'Mesin On' : 'Mesin Off';
        }
    </script>
@endsection
